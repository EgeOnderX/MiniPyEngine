...
def load_mtl_file(file_location, file_name, mesh_model):
    print("[INFO]  Start loading MTL: " + file_name)
    mtl = None
    ...
    for line in fin.readlines():
        ...
        if tokens[0] == "newmtl":
            ...
            is_image_file = True
        elif not tokens[0].startswith('#') and tokens[0] not in {"newmtl", "Kd", "Ks", "Ns", "map_Kd"}:
            print(f"[INFO] Unknown MTL token: {tokens[0]}")
    print("[INFO]  Finished loading MTL: " + file_name)

def load_obj_file(file_location, file_name):
    print("[INFO]  Start loading OBJ: " + file_name)
    ...
    for line in fin.readlines():
        ...
        if tokens[0] == "mtllib":
            ...
            print(f"[INFO] Unknown OBJ token: {tokens[0]}")
    mesh_model.set_opengl_buffers()
    print("[INFO] Finished loading OBJ: " + file_name)
    return mesh_model
