...
class GameObject:
    ...
    def collision(self, object: GameObject) -> GameObject:
        ...
        y1 = self.position.y + (self.scale.y / self.collision_resize + 0.1)
        y2 = self.position.y - (self.scale.y / self.collision_resize + 0.1)

        z1 = self.position.z + (self.scale.z / self.collision_resize + 0.2)
        z2 = self.position.z - (self.scale.z / self.collision_resize + 0.2)

        # 3D AABB
        if (p_x <= x1 and p_x >= x2 and
                p_y <= y1 and p_y >= y2 and
                p_z <= z1 and p_z >= z2):
